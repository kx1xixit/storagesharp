...
// Also extract associated blockType to identify EVENT blocks (hats triggered via startHats).
// For EVENT blocks, no handler method is required.
const opcodeMatches = [...strippedSource.matchAll(/opcode\s*:\s*['"]([^'"]+)['"]/g)];
const declaredOpcodes = opcodeMatches.map(m => m[1]);

// Build a set of opcodes that are EVENT-type blocks (no method handler required).
// Look for object literals containing both an opcode and a blockType of EVENT.
// We scan a tight window (~150 chars before and after the opcode) to stay within
// the same block object and avoid picking up adjacent blocks' blockType values.
const eventOpcodes = new Set();
for (const m of opcodeMatches) {
  const start = Math.max(0, m.index - 150);
  const end = Math.min(strippedSource.length, m.index + 150);
  const ctx = strippedSource.slice(start, end);
  if (/blockType\s*:\s*(?:Scratch\.BlockType\.EVENT\b|['"]event['"])/i.test(ctx)) {
    eventOpcodes.add(m[1]);
  }
}
...
if (declaredOpcodes.length === 0) {
  ...
  for (const opcode of declaredOpcodes) {
    if (eventOpcodes.has(opcode)) {
      pass(`Block "${opcode}" is an EVENT block (no handler method required)`);
    } else if (definedMethods.has(opcode)) {
